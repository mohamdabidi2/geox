...
const GestionDroitsMagasins = () => {
  ...
  const fetchAllData = async () => {
    try {
      ...
      const [rightsResponse, usersResponse, magasinsResponse] = await Promise.all([
        axios.get('http://localhost:5000/api/droit-by-magasin'),
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (editingRight) {
        ...
        const response = await axios.put(`http://localhost:5000/api/droit-by-magasin/${editingRight.id}`, {
        ...
        // Create new right
        const response = await axios.post('http://localhost:5000/api/droit-by-magasin', dataToSubmit);
  ...
  const handleDelete = async (rightId) => {
    if (window.confirm('Êtes-vous sûr de vouloir supprimer ce droit magasin ?')) {
      try {
        const response = await axios.delete(`http://localhost:5000/api/droit-by-magasin/${rightId}`);
